...
gesture timing, first-cut-to-resolve hesitation, last-cut-to-resolve hesitation,
resolve commits, release-latched cuts, adjusted cuts, final cut ownership,
gesture samples, and no-cut acknowledgements. `responses` counts accepted cut
responses during the round, including adjustment churn; `owned-cuts` is the final
set of cuts owned by the last gesture. `near` means the tester was aiming near a
valid slot, while `off` means the swipe landed outside the cut vocabulary.
`last-source` reports whether the final submitted cut came from direct contact,
a pointer-up release latch, an adjacent-slot adjustment, or no cut at all.
They also report touch-loupe samples, snap-ready loupe samples, low-clearance
loupe samples, and minimum pointer clearance so finger-occlusion reports can be
checked against the game's own touch-aim evidence.
