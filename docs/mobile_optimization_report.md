...
- Compact active-play controls now use the same exit semantics across tutorial and endless: mobile tutorial changed from `Menu` to `Exit`, preserving the documented Undo / Clear / Exit / Resolve control contract and avoiding a misleading menu-surface label during live play.
