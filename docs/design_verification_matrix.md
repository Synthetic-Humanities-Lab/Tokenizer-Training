...
This matrix separates the current source contract from the June 2026
verification record. Use the snapshot below and
`docs/current_surface_contract.md` for present-surface decisions. Preserve the
later record as provenance rather than silently converting its observations
into current requirements.

## Current Snapshot - 2026-07-18

- Browsers and the iOS shell run the same Phaser/Vite game. The mobile profile
  adapts safe areas, layout, and touch reach without forking mechanics or content.
- The mobile HUD shows `CREDITS`, `TIME`, `BEST RUN`, and `CURRENT`; browser
  layouts may retain verified/rework and progress fields.
- Review uses one feedback card for resolved token boundaries, verified/rework
  Token Credit accounting, net change, and cut-audit evidence. Token IDs remain
  on the falling resolved pieces rather than in the card, and its generated
  technical descriptor stays hidden.
- One Wiener and one Wiener speech bubble carry the live character/instruction
  surface. Detached tutorial popups, robot toasts, overseer panels, and separate
  token strips are retired.
- Play controls read left-to-right as `Sound`, `Clear`, `Exit`, and `Resolve`,
  with the final action changing to `Continue` or `Finish` when appropriate.

Evidence freshness and user comprehension remain separate gates; this snapshot
defines the current surface, not proof that every current route has fresh
browser or physical-device evidence.

## Historical June Verification Record

Everything below is the preserved June 2026 chronology, requirements matrix,
principle assessment, and proposed evidence plan. Terms such as "current",
"latest", and "now" describe the state at the time of those entries. Legacy
surface names, IDs, strings, counts, screenshots, and statuses are historical
observations and do not override the current snapshot above.
...
| Real tokenizer fixtures used | `scripts/generate-token-fixtures.ts`, 200 checked-in `cl100k_base` fixtures, tokenizer fixture tests | Met |
...
| Numerical-token comprehension probe exists | `docs/token_comprehension_probe.md` defines a separate, unscored, two-form near-transfer protocol for words-versus-chunks, leading-space ownership, and encoding-specific token IDs; `tests/token-comprehension-probe.test.ts` verifies exact real tokenizer chunks and IDs, the non-game/non-progression boundary, rubric, and four-of-five threshold; this is a prepared instrument, not participant evidence | Met |
...
| User comprehension proven | no external tester observations yet; the tokenizer-verified external probe is prepared, but no completed probe record exists | Not met |
| Fixture variety suitable for extended training | 200 checked-in `cl100k_base` fixtures across tiers 1-4; regression tests protect the exact 30/30/20/20 content mix, 40/50/50/60 tier distribution, unique one-line prompts, fictional model-company brands, and playable grapheme boundaries | Met |

### Historical Principle Embodiment
...
review markers, the canonical feedback card, pay, cost, net, rank, and copied
start-source evidence. Tutorial instruction has exactly two live Wiener speech
paths: one sticky active-round instruction and one outcome-specific review line.
The second review line adds one encoding-qualified `cl100k_base` ID sample, while
the feedback card and Token Log retain the deeper chunk and ID evidence. Historical
staged mechanics, byte-route, token-ID, work-rule, follow-up, and review-window
APIs are not part of the production teaching sequence.
...
### Historical Next Evidence To Collect
